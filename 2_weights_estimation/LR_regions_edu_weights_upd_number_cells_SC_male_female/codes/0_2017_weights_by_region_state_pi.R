...
#
# Usage:
#   Rscript 0_2017_weights_by_region_state_pi.R --region east
#   Rscript 0_2017_weights_by_region_state_pi.R --region west
# ===============================================================
...
if (!length(region_arg_pos) || length(args) < region_arg_pos + 1) {
  stop("Usage: Rscript 0_2017_weights_by_region_state_pi.R --region east|west")
}
...
# w2 grid — separate for males and females (w1 fixed at 1)
W1_FIXED       <- 1
W2_GRID_MALE   <- c(0.25, 0.5, 1, 2, 4)
W2_GRID_FEMALE <- c(0.25, 0.5, 1, 2, 4)
...
# Global appended CSV
DIAG_CSV <- "../../weights_output/w2_grid_diagnostics_by_region_state_pi_SC_gender.csv"
...
MALE_KEYS   <- group_spec$key[grepl("^male_",   group_spec$key)]
FEMALE_KEYS <- group_spec$key[grepl("^female_", group_spec$key)]
...
build_region_capacity_edu <- function(inter_sf, muni_all, group_spec, E_data, SC_PARTIES,
  ...
  if (!is.null(gkeys_filter)) group_spec <- group_spec[group_spec$key %in% gkeys_filter, ]
  K     <- 6L
  nc    <- nrow(inter_sf)
  R     <- nrow(muni_all)
...
# GENDER-SPLIT: males and females have separate Voronoi partitions,
# so Pi_male[s][gk] is over nc_male cells and Pi_female[s][gk]
# is over nc_female cells.
#
# Prediction per row r (state s_r):
#   pred_r = sum_{gk in male_keys}  B_male[gk][r,] . Pi_male[s_r][gk]
#          + sum_{gk in female_keys} B_female[gk][r,] . Pi_female[s_r][gk]
#
# Constraints are applied within each gender separately.
# ============================================================
solve_afd_only_region_state_pi <- function(cap_male,
                                           cap_female,
                                           tg_by_state,
                                           sc_by_state,
                                           AFD_GROUP_TOL,
                                           SC_TOL,
                                           EDU_DIFF_TOL,
                                           prev_Pi_vals_male   = NULL,
                                           prev_Pi_vals_female = NULL,
                                           nc_prev_male        = NULL,
                                           nc_prev_female      = NULL) {

  male_keys   <- names(cap_male$B_AFD_list)
  female_keys <- names(cap_female$B_AFD_list)

  # Shared across both caps (derived from same muni_all)
  share_afd      <- cap_male$share_afd
  weights_pop    <- cap_male$weights_pop
  states_present <- cap_male$states_present
  states_chr     <- as.character(states_present)
  state_row_idx  <- cap_male$state_row_idx

  R    <- length(share_afd)
  nc_m <- ncol(cap_male$B_AFD_list[[1]])
  nc_f <- ncol(cap_female$B_AFD_list[[1]])

  base_keys_male   <- unique(gsub("_edu$", "", male_keys))
  base_keys_female <- unique(gsub("_edu$", "", female_keys))

  # ----- CVXR variables -----
  Pi_male <- setNames(lapply(states_chr, function(sid) {
    setNames(lapply(male_keys,   function(gk) CVXR::Variable(nc_m)), male_keys)
  }), states_chr)
  Pi_female <- setNames(lapply(states_chr, function(sid) {
    setNames(lapply(female_keys, function(gk) CVXR::Variable(nc_f)), female_keys)
  }), states_chr)

  # Warm-start helper
  warm_init <- function(Pi_set, prev_vals, nc, nc_prev) {
    if (is.null(prev_vals)) return(invisible(NULL))
    nc_match <- !is.null(nc_prev) && (nc_prev == nc)
    for (sid in names(Pi_set)) {
      pv_s <- prev_vals[[sid]]
      if (is.null(pv_s)) next
      for (gk in names(Pi_set[[sid]])) {
        pv_gk <- pv_s[[gk]]
        if (is.null(pv_gk) || !is.numeric(pv_gk)) next
        if (nc_match && length(pv_gk) == nc) {
          init_val <- pmax(pv_gk, 0); s <- sum(init_val)
          if (is.finite(s) && s > 0) init_val <- init_val / s
          CVXR::value(Pi_set[[sid]][[gk]]) <- init_val
        } else {
          CVXR::value(Pi_set[[sid]][[gk]]) <- rep(1/nc, nc)
        }
      }
    }
  }
  warm_init(Pi_male,   prev_Pi_vals_male,   nc_m, nc_prev_male)
  warm_init(Pi_female, prev_Pi_vals_female, nc_f, nc_prev_female)
  ...
  for (sid in states_chr) {
    ...
    pred_m <- Reduce(`+`, lapply(male_keys, function(gk) {
      B_s <- cap_male$B_AFD_list[[gk]][idx_s, , drop = FALSE]
      B_s %*% Pi_male[[sid]][[gk]]
    }))
    pred_f <- Reduce(`+`, lapply(female_keys, function(gk) {
      B_s <- cap_female$B_AFD_list[[gk]][idx_s, , drop = FALSE]
      B_s %*% Pi_female[[sid]][[gk]]
    }))
    pred_parts[[sid]] <- pred_m + pred_f
  }

  pred_parts <- pred_parts[!sapply(pred_parts, is.null)]
  if (!length(pred_parts)) return(list(status = "no_states"))

  pred <- Reduce(CVXR::vstack, pred_parts)
  u    <- CVXR::Variable(R)

  cons <- list(u >= 0, pred - share_afd_ord <= u, -(pred - share_afd_ord) <= u)

  # Positivity + normalisation
  for (sid in states_chr) {
    for (gk in male_keys)
      cons <- c(cons, list(Pi_male[[sid]][[gk]]   >= 0,
                           CVXR::sum_entries(Pi_male[[sid]][[gk]])   == 1))
    for (gk in female_keys)
      cons <- c(cons, list(Pi_female[[sid]][[gk]] >= 0,
                           CVXR::sum_entries(Pi_female[[sid]][[gk]]) == 1))
  }

  # Per-state, per-gender constraints helper
  add_gender_cons <- function(cap_g, Pi_g, base_keys_g, all_gkeys_g, sid, tg_state, sc_state) {
    gm_s       <- cap_g$gm_by_state[[sid]]
    rows_afd_s <- cap_g$rows_afd_state[[sid]]
    rows_sc_s  <- cap_g$rows_sc_state[[sid]]
    new_cons   <- list()
    for (bg in base_keys_g) {
      sub_keys <- intersect(c(bg, paste0(bg, "_edu")), all_gkeys_g)
      ...
      lhs_parts <- lapply(sub_keys, function(gk)
        t(matrix(rows_afd_s[[gk]], ncol = 1)) %*% Pi_g[[sid]][[gk]])
      lhs <- Reduce(`+`, lhs_parts)
      new_cons <- c(new_cons, list(lhs >= (X_bg * T_total - tol_abs),
      ...
      if (is.finite(sc_target) && sc_target >= 0 && X_bg > 0) {
        sc_parts <- lapply(sub_keys, function(gk)
          t(matrix(rows_sc_s[[gk]][["5"]], ncol = 1)) %*% Pi_g[[sid]][[gk]])
        lhs_sc      <- Reduce(`+`, sc_parts)
        target_mass <- sc_target * X_bg * T_total
        tol_sc      <- SC_TOL    * X_bg * T_total
        new_cons <- c(new_cons, list(lhs_sc >= (target_mass - tol_sc),
    ...
    for (bg in base_keys_g) {
      ...
      if (!(edu_key %in% all_gkeys_g)) next
      new_cons <- c(new_cons, list(
        Pi_g[[sid]][[bg]]      <= (1 + EDU_DIFF_TOL) * Pi_g[[sid]][[edu_key]],
        Pi_g[[sid]][[edu_key]] <= (1 + EDU_DIFF_TOL) * Pi_g[[sid]][[bg]]
      ))
    }
    new_cons
  }
  ...
  for (sid in states_chr) {
    ...
    cons <- c(cons, add_gender_cons(cap_male,   Pi_male,   base_keys_male,   male_keys,   sid, tg_state, sc_state))
    cons <- c(cons, add_gender_cons(cap_female, Pi_female, base_keys_female, female_keys, sid, tg_state, sc_state))
  }

  prob     <- CVXR::Problem(CVXR::Minimize(CVXR::sum_entries(u)), cons)
  use_warm <- !is.null(prev_Pi_vals_male) || !is.null(prev_Pi_vals_female)
  ...
  Pi_vals_male <- setNames(lapply(states_chr, function(sid) {
    setNames(lapply(male_keys,   function(gk) as.numeric(res$getValue(Pi_male[[sid]][[gk]]))),   male_keys)
  }), states_chr)
  Pi_vals_female <- setNames(lapply(states_chr, function(sid) {
    setNames(lapply(female_keys, function(gk) as.numeric(res$getValue(Pi_female[[sid]][[gk]]))), female_keys)
  }), states_chr)
  ...
  list(
    ...
    Pi_vals_male    = Pi_vals_male,
    Pi_vals_female  = Pi_vals_female,
    mae             = mae,
    residuals       = resid_v,
...
# ============================================================
# Build the intersected Voronoi mesh for a single w2 value
# Returns inter_sf2 (sf object with p_e*, sc_e* columns).
# ============================================================
build_inter_sf_for_w2 <- function(C_list, E_data, xy_obs, w2) {
...
run_one_region_w2 <- function(region_label, STATES, seed_base = 0, seed_idx = 1) {
  ...
  # ----- 2D grid search (w2_male × w2_female) with warm start -----
  W2_PAIRS <- expand.grid(w2_male   = W2_GRID_MALE,
                          w2_female = W2_GRID_FEMALE,
                          stringsAsFactors = FALSE)

  message(sprintf(
    "\n[W2 GRID SEARCH] YEAR %d | %s | grid_male=%s | grid_female=%s | n_states=%d | max_elections/state=%d | subsample_n=%d\n",
    YEAR, toupper(region_label),
    paste(W2_GRID_MALE,   collapse = ","),
    paste(W2_GRID_FEMALE, collapse = ","),
    length(STATES), MAX_ELECTIONS_PER_STATE, SUBSAMPLE_N
  ))

  # Pre-build partitions for all unique w2 values (avoids duplicate geometry work)
  all_w2_unique   <- sort(unique(c(W2_GRID_MALE, W2_GRID_FEMALE)))
  partition_cache <- list()
  for (w2v in all_w2_unique) {
    message(sprintf("[PARTITION] Building partition for w2=%.3f ...", w2v))
    prt <- tryCatch(
      build_inter_sf_for_w2(C_list, E_data, xy_obs, w2v),
      error = function(e) {
        message(sprintf("[PARTITION] Error for w2=%.3f: %s", w2v, conditionMessage(e)))
        NULL
      }
    )
    partition_cache[[as.character(w2v)]] <- prt
  }

  grid_rows           <- list()
  best                <- list(w2_male = NA_real_, w2_female = NA_real_, score = Inf,
                              status = NA_character_, row = NULL, fit = NULL,
                              inter_sf_male = NULL, inter_sf_female = NULL,
                              nc_male = NA_integer_, nc_female = NA_integer_)
  prev_Pi_vals_male   <- NULL
  prev_Pi_vals_female <- NULL
  prev_nc_male        <- NULL
  prev_nc_female      <- NULL

  for (pair_i in seq_len(nrow(W2_PAIRS))) {
    w2_male   <- W2_PAIRS$w2_male[pair_i]
    w2_female <- W2_PAIRS$w2_female[pair_i]
    message(sprintf("[W2 TRY] YEAR %d | %s | w2_male=%.3f | w2_female=%.3f",
                    YEAR, toupper(region_label), w2_male, w2_female))

    out <- tryCatch({
      inter_sf_male   <- partition_cache[[as.character(w2_male)]]
      inter_sf_female <- partition_cache[[as.character(w2_female)]]
      if (is.null(inter_sf_male))
        stop(sprintf("No cached partition for w2_male=%.3f", w2_male))
      if (is.null(inter_sf_female))
        stop(sprintf("No cached partition for w2_female=%.3f", w2_female))

      nc_m <- nrow(inter_sf_male)
      nc_f <- nrow(inter_sf_female)
      message(sprintf(
        "[MESH] Region %s, year %d, w2_male=%.3f/w2_female=%.3f: %d male cells, %d female cells, %d elections",
        toupper(region_label), YEAR, w2_male, w2_female, nc_m, nc_f, E_data))

      cap_male   <- build_region_capacity_edu(inter_sf_male,   muni_all, group_spec,
                                              E_data, SC_PARTIES,
                                              gkeys_filter = MALE_KEYS)
      cap_female <- build_region_capacity_edu(inter_sf_female, muni_all, group_spec,
                                              E_data, SC_PARTIES,
                                              gkeys_filter = FEMALE_KEYS)

      fit <- solve_afd_only_region_state_pi(
        cap_male            = cap_male,
        cap_female          = cap_female,
        tg_by_state         = tg_by_state,
        sc_by_state         = sc_by_state,
        AFD_GROUP_TOL       = AFD_GROUP_TOL,
        SC_TOL              = SC_TOL,
        EDU_DIFF_TOL        = EDU_DIFF_TOL,
        prev_Pi_vals_male   = prev_Pi_vals_male,
        prev_Pi_vals_female = prev_Pi_vals_female,
        nc_prev_male        = prev_nc_male,
        nc_prev_female      = prev_nc_female
      )

      list(inter_sf_male = inter_sf_male, inter_sf_female = inter_sf_female,
           cap_male = cap_male, cap_female = cap_female,
           fit = fit, nc_male = nc_m, nc_female = nc_f)
    }, error = function(e) list(error = conditionMessage(e)))

    if (!is.null(out$error)) {
      grid_rows[[length(grid_rows) + 1L]] <- data.frame(
        year = YEAR, geo_region = region_label, w1 = W1_FIXED,
        w2_male = w2_male, w2_female = w2_female,
        status = "error",
        n_cells_male = NA_integer_, n_cells_female = NA_integer_,
        E_data = E_data, n_obs = R,
    ...
    if (!(status %in% c("optimal","optimal_inaccurate"))) {
      grid_rows[[length(grid_rows) + 1L]] <- data.frame(
        ...
        w2_male = w2_male, w2_female = w2_female,
        status = status,
        n_cells_male = as.integer(nc_m), n_cells_female = as.integer(nc_f),
        E_data = E_data, n_obs = R,
        mae = NA_real_, wmae = NA_real_,
        mae_per_cell = NA_real_, wmae_per_cell = NA_real_,
        mae_plus_lambda_cells = NA_real_, wmae_plus_lambda_cells = NA_real_,
    ...
    n_cells_total <- nc_m + nc_f
    mets <- compute_fit_metrics(out$fit, n_cells_total, lambda_cells = LAMBDA_CELLS)
    ...
    message(sprintf("  -> status=%s | MAE=%.6f | WMAE=%s | cells_male=%d | cells_female=%d",
                    status, mets$mae,
                    if (is.finite(mets$wmae)) sprintf("%.6f", mets$wmae) else "NA",
                    nc_m, nc_f))

    # Update warm start for next pair
    prev_Pi_vals_male   <- out$fit$Pi_vals_male
    prev_Pi_vals_female <- out$fit$Pi_vals_female
    prev_nc_male        <- nc_m
    prev_nc_female      <- nc_f
    ...
    if (is.finite(score) && score < best$score) {
      best <- list(w2_male = w2_male, w2_female = w2_female, score = score,
                   status = status, row = row, fit = out$fit,
                   inter_sf_male = out$inter_sf_male, inter_sf_female = out$inter_sf_female,
                   cap_male = out$cap_male, cap_female = out$cap_female,
                   nc_male = nc_m, nc_female = nc_f)
    }
  }  # end w2 pair loop
  ...
  if (!is.finite(best$score) || is.na(best$w2_male)) {
    message(sprintf("[FAIL] No feasible w2 pair found for region %s, year %d",
                    region_label, YEAR))
    return(list(
  ...
  message(sprintf("\n[W2 BEST] YEAR %d | %s | best w2_male=%.3f | w2_female=%.3f | score=%s\n",
                  YEAR, toupper(region_label), best$w2_male, best$w2_female,
  ...
  list(
    ...
    feasible        = TRUE,
    grid_df         = grid_df,
    w2_male_best    = best$w2_male,
    w2_female_best  = best$w2_female,
    best_score      = best$score,
    best_row        = best$row,
    E_data          = E_data,
    n_cells_male    = best$nc_male,
    n_cells_female  = best$nc_female,
    Pi_vals_male    = best$fit$Pi_vals_male,
    Pi_vals_female  = best$fit$Pi_vals_female,
    residuals       = best$fit$residuals,
    mae             = best$fit$mae,
    inter_sf_male   = best$inter_sf_male,
    inter_sf_female = best$inter_sf_female,
    muni_all        = muni_all,
    tg_by_state     = tg_by_state,
    sc_by_state     = sc_by_state
  )
...
for (YEAR in YEARS) {
  ...
  for (seed_idx in seq_along(SEED_LIST)) {
    ...
    saveRDS(list(
      ...
      seed_base      = seed_base,
      w1             = W1_FIXED,
      w2_grid_male   = W2_GRID_MALE,
      w2_grid_female = W2_GRID_FEMALE,
      result         = res,
      AFD_GROUP_TOL  = AFD_GROUP_TOL,
      SC_TOL         = SC_TOL,
      EDU_DIFF_TOL   = EDU_DIFF_TOL,
