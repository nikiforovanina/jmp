# ===============================================================
# 0_weights_by_region_state_pi.R — unified runner
# w2 grid search, 24-group edu model, state-specific Pi
#
# Usage (sourced by 0_region_state_pi_<region>_<YEAR>.sh):
...
message(sprintf(">>> Running 0_weights_by_region_state_pi.R | YEAR=%d | REGION=%s",
                YEAR, REGION_RUN))
...
for (seed_idx in seq_along(SEED_LIST)) {
  ...
  out_rds <- sprintf("fits_afd_only/afd_only_region_state_pi_year%d_%s_seed%02d_%s.rds",
                     YEAR, region_label, seed_idx, SPEC_TAG)
  saveRDS(list(
    year          = YEAR,
