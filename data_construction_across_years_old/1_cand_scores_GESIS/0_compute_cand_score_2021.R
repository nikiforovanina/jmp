...
scores_2021_not_shrinked <- compute_scores_2021(cand21) %>%
  group_by(bula) %>%
  mutate(
...
scores_2021_not_shrinked <- scores_2021_not_shrinked %>% filter(!is.na(party_id))
...
cand21_scored <- cand21 %>%
  ...
  left_join(scores_2021_not_shrinked %>% select(wknr, a1_or_partei, econ_lr_score, anti_elite_score, party_id),
