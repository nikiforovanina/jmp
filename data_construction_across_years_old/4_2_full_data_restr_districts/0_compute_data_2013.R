...
# =========================================================
load("../output_data/census_2013.Rdata")  # provides census_shares_only
census <- census_shares_2013

# *** RESTORED: your previous normalization logic ***
# keep 1–4, drop 5–8, then keep the rest
...
# left-pad to 8 for safety (some workflows expect 8-digit AGS)
census$ags <- stringr::str_pad(census$ags, 8, pad = "0")

# =========================================================
# Elections (all years; we’ll slice 2013)
# =========================================================
...
cand13_scored_full_shrunk <- cand13_scored_full_shrunk %>% group_by(wknr) %>% mutate(n_wknr = n()) %>% ungroup()
cand13_scored_full_shrunk <- cand13_scored_full_shrunk %>% filter(n_wknr > 3)
cand13_scored_full_shrunk$n_wknr <- NULL
...
build_state_2013 <- function(STATE_ID, out_dir = "data") {
  ...
  if (nrow(e17) == 0) { message("  [SKIP] no municipalities"); return(invisible(NULL)) }
  
  # AGS left-pad to 8
  e17$ags    <- str_pad(as.character(e17$ags), 8, pad = "0")
  census$ags <- str_pad(as.character(census$ags), 8, pad = "0")
  
  
  # ---- keep overlap (Census ∩ Elections) ----
  muni_base <- census %>% inner_join(e17, by = "ags")
  ...
  wk_map <- readr::read_csv2(
    ...
    mutate(
      land  = str_pad(Land,             2, pad = "0"),  # 2 digits
      rb    = str_pad(Regierungsbezirk, 1, pad = "0"),  # 1 digit (0 if none)
      kreis = str_pad(Kreis,            2, pad = "0"),  # 2 digits
      gem   = str_pad(Gemeinde,         3, pad = "0"),  # 3 digits
  ...
  municipalities_small <- municipalities_small %>%
    mutate(
      # use normalized vote shares if available
...
message("\nDone for 2013 with improved candidate handling (WK -> state -> national fallbacks).")
