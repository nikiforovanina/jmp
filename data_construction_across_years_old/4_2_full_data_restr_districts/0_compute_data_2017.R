# =========================================================
# Build results_std / pos_std for ALL states (1..16) for YEAR
# =========================================================
...
# ---------- helpers ----------
wk_file_for_year <- function(Y) {
  # adjust here if you later add other years
  if (Y == 2017) return("btw17_wkr_gemeinden.csv")
  stop(sprintf("No WK->AGS file mapping defined for year %d", Y))
}
...
# Ensures cand1..cand6 shares sum to 1 (proportional rescale if sum > 0)
...
# =========================================================
# Census -> demographic shares
# =========================================================
load("../output_data/census_2017.Rdata")  # provides census_shares_only
census <- census_shares_2017

# keep 1–4, drop 5–8, then keep the rest (restored logic)
normalize_code <- function(code) {
  ...
  out <- ifelse(
    n == 12,
    # 12-digit RS: keep 1–5 and 10–12
    paste0(substr(code, 1, 5), substr(code, 10, 12)),
    ifelse(
      n == 11,
      # 11-digit RS: your rule – keep 1–4 and 9–end
      paste0(substr(code, 1, 4), substr(code, 9, n)),
      ifelse(
        n == 8,
        # already AGS
        code,
...
# =========================================================
load("../output_data/cand17_scored_full_shrunk.Rdata")    

cand17_scored_full_shrunk <- cand17_scored_full_shrunk %>% group_by(wknr) %>% mutate(n_wknr = n()) %>% ungroup()
cand17_scored_full_shrunk <- cand17_scored_full_shrunk %>% filter(n_wknr > 3)
cand17_scored_full_shrunk$n_wknr <- NULL

dat_nat <- cand17_scored_full_shrunk
...
# =========================================================
# One-state builder (YEAR-agnostic)
# =========================================================
build_state <- function(STATE_ID, out_dir = "data") {
  message(sprintf("\n=== YEAR %d | STATE %02d ===", YEAR, STATE_ID))
  
  wk_file <- wk_file_for_year(YEAR)
  if (!file.exists(wk_file)) {
    message(sprintf("  [SKIP] Missing WK->AGS file: %s", wk_file)); return(invisible(NULL))
  }
  ...
  # =========================================================
  # Candidate medians with 3-level fallback (WK -> State -> National)
  # =========================================================
  ...
  # =========================================================
  # results_std (shares) + pos_std (positions)
  # =========================================================
  ...
  make_results_std <- function(municipalities_small) {
    party_map <- c(
    ...
    dem_cols <- c(
      "male_18_24_share","male_25_34_share","male_35_44_share",
      "male_45_59_share","male_60_69_share","male_70plus_share",
      "female_18_24_share","female_25_34_share","female_35_44_share",
      "female_45_59_share","female_60_69_share","female_70plus_share"
    )
    
    municipalities_small %>%
      mutate(
  ...
  out_file <- file.path(out_dir, sprintf("data_state%02d_year%d.RData", STATE_ID, YEAR))
...
# ---------- run for all states ----------
invisible(lapply(STATES, function(st) try(build_state(st), silent = FALSE)))
message("\nDone for 2017 with improved candidate handling (WK -> state -> national fallbacks).")

save(dat_nat, file = "data/full_cand_position_17.RData")
