# =========================================================
# Build results_std / pos_std for ALL states (1..16) in 2021
# Uses education-split census shares (census_2021_edu.Rdata)
# =========================================================
...
# Per-mode output directory: each CAND_SCORE_MODE writes its per-state-per-year
# edu .RData files into a separate folder so re-runs with different modes
# do not overwrite previous outputs. CAND_SCORE_MODE is set by
# 0_data_construction.R; if running this script directly, defaults to "pooled".
.cand_mode <- Sys.getenv("CAND_SCORE_MODE", unset = "pooled")
.out_dir   <- sprintf("data_%s", .cand_mode)
dir.create(.out_dir, showWarnings = FALSE, recursive = TRUE)
message(sprintf(">>> [edu 2021] writing to %s/", .out_dir))

YEAR   <- 2021
STATES <- 1:16
SMALL_MUNI_MAX_ELIGIBLE <- 10000000

# ---------- helpers ----------
wk_file_for_year <- function(Y) {
  if (Y == 2021) return("btw21_wkr_gemeinden.csv")
...
# =========================================================
load("../data/census_2021_edu.Rdata")   # → census_2021_edu
census <- census_2021_edu
...
dat_nat        <- cand21_scored_full_shrunk
...
# One-state builder
# =========================================================
build_state <- function(STATE_ID, out_dir = .out_dir) {
  ...
  # `imputed_fallback` = 1 when the (wknr, party) position is NOT from a
  # real candidate cohort in that Wahlkreis (no direct GESIS row and no
  # wen-waehlen row, possibly elastic-net imputed). In that case the value
  # below comes from the state median, the national median, or — last resort —
  # the grid-wide backup median. = 0 when at least one real candidate of that
  # party existed in the WK and contributed to wk_party_medians.
  grid <- tidyr::expand_grid(wknr = keep_wk, party_id = 1:6) %>%
    ...
    mutate(
      imputed_fallback = as.integer(is.na(econ_wk) | is.na(elite_wk)),
      econ_lr_score    = coalesce(econ_wk,  econ_state,  econ_nat),
      anti_elite_score = coalesce(elite_wk, elite_state, elite_nat)
    ) %>%
    select(wknr, party_id, econ_lr_score, anti_elite_score, imputed_fallback)
  ...
  if (anyNA(grid$econ_lr_score) || anyNA(grid$anti_elite_score)) {
    ...
    grid <- grid %>%
      mutate(
        # Anything still NA after the WK->state->nat coalesce is a grid-level
        # backup fill — also flagged as imputed_fallback (it was already 1
        # because is.na(econ_wk) was true; this only matters for cells where
        # state and nat medians were both NA too).
        imputed_fallback = ifelse(is.na(econ_lr_score) | is.na(anti_elite_score),
                                  1L, imputed_fallback),
  ...
  pos_std <- grid %>%
    transmute(
      ...
      x                = econ_lr_score,
      y                = anti_elite_score,
      imputed_fallback = imputed_fallback
    ) %>%
    distinct()
...
save(dat_nat, file = "data/full_cand_position_21.RData")
