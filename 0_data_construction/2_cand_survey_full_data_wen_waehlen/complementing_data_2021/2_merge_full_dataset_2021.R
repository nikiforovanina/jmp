...
.cand_suffix <- switch(.cand_mode,
  pooled = "pooled", irt = "irt", legacy = "pooled",
  z_capped_1 = "pooled", z_capped_1_5 = "pooled", "LR")
load(sprintf("../../1_cand_scores_GESIS/data/cand21_scored_%s.Rdata", .cand_suffix))
...
cand21_scored_full <- rbind(cand21_scored_pid, wen_to_merge)
...
cand21_scored_full <- cand21_scored_full |>
  dplyr::left_join(party_labels, by = "party_id") |>
  dplyr::select(-party_id)  # keep only the new 'partei' column

save(cand21_scored_full, file = "../data/cand21_scored_full.Rdata")
...
shrink_to_party <- function(df_scored,
                            ...
                            clamp_min   = -1.5,  # clamp range after shrink
                            clamp_max   =  1.5,
                            medians_df  = NULL) {
  # `medians_df`: optional tibble with columns party_id, econ_med, elite_med.
  # When supplied, the shrink anchor is taken from this table instead of
  # being computed from `df_scored`. We use this to anchor on direct GESIS
  # medians only — otherwise imputed candidates contaminate the median that
  # they themselves are shrunk toward (the AfD amplification loop).
  ...
  if (is.null(medians_df)) {
    ...
    stopifnot(all(c("party_id","econ_med","elite_med") %in% names(medians_df)))
    meds <- medians_df
  }
...
# ---------- apply shrink & save ----------
# (3) Decouple the shrink anchor from imputed values: compute party medians
# from direct GESIS scores only (`cand21_scored_pid`) and pass them in. Using
# the mixed (direct + imputed) `cand21_scored_full` lets imputation drift
# pull the anchor toward whatever the elastic net produced — a feedback loop
# that amplified AfD's old↔new shift to +0.37.
direct_medians <- cand21_scored_pid %>%
  group_by(party_id) %>%
  summarise(
    econ_med  = median(econ_lr_score,    na.rm = TRUE),
    elite_med = median(anti_elite_score, na.rm = TRUE),
...
# In "legacy" mode, recompute the shrink anchor from mixed (direct + imputed)
# data — matches data_construction_across_years_old's behaviour.
.legacy_shrink <- identical(.cand_mode, "legacy")
cand21_scored_full_shrunk <- shrink_to_party(
  df_scored   = cand21_scored_full,
  lut         = party_lut_codes,
  max_dev     = 0.5,
  lambda_with = 0.75,
  clamp_min   = -1.5,
  clamp_max   =  1.5,
  medians_df  = if (.legacy_shrink) NULL else direct_medians
)



save(cand21_scored_full_shrunk,   file = "../data/cand21_scored_full_shrunk.Rdata")
...
cand21_scored_full_shrunk <- cand21_scored_full_shrunk %>%
...
cand21_scored_plot <- ggplot(
  cand21_scored_full_shrunk,
...
cand21_scored_plot
ggsave("../graphs/plot_can_2021_LR.png", cand21_scored_plot, width = 7, height = 5, dpi = 300)
