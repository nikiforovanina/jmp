...
# 1) Read cand13 + LUT
#----------------------------
cand13 <- haven::read_dta("../../1_cand_scores_GESIS/candidates_2013.dta")
...
cand13$partei <- cand13$a1
# Map to party_id, keep only 6 main, presence per (wknr, party_id)
cand13$wknr = cand13$wkname
cand13_mapped <- cand13 %>%
  mutate(wknr = as.numeric(wkname)) %>%
  left_join(party_lut_codes, by = c("partei" = "orig")) %>%
  filter(!is.na(party_id), !is.na(wknr), wknr > 0) %>%
  distinct(wknr, party_id) %>%
  mutate(exist = 1L)

# Complete grid for cand13
wknr_all <- cand13 %>% mutate(wknr = as.numeric(wknr)) %>%
...
df_exist <- tidyr::expand_grid(
  ...
  left_join(cand13_mapped, by = c("wknr","party_id")) %>%
...
#----------------------------
wen_waehlen <- read.csv("wen_waehlen_btw13_candidates.csv")
...
#    (keeps the cand13 universe of wknr; switch to full_join if you want union)
#----------------------------
df_exist <- df_exist %>%
  left_join(
...
# cand13_scored columns assumed: partei, wknr, econ_lr_score, anti_elite_score

cand13_scored_collapsed <- cand13_scored %>%
  mutate(
...
wen_merged <- wen_exist_grid %>%
  left_join(cand13_scored_collapsed, by = c("wknr","party_id"))
...
# Persist for downstream 1_imputing_positions_2013.R
save(wen_merged, file = "wen_merged_2013.Rdata")
