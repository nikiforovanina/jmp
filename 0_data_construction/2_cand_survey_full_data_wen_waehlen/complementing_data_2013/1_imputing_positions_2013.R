...
# wen_merged is produced by 0b_data_complement_2013_cand_survey_wen.R
load("wen_merged_2013.Rdata")
...
write.csv(econ_hp, "../data/hyperparams_2013_econ.csv", row.names = FALSE)
write.csv(anti_hp, "../data/hyperparams_2013_anti.csv", row.names = FALSE)
message(sprintf("[2013 econ] best: penalty=%g  mixture=%g  cv_rmse=%.3f  cv_rsq=%.3f",
                econ_hp$penalty, econ_hp$mixture, econ_hp$cv_rmse, econ_hp$cv_rsq))
message(sprintf("[2013 anti] best: penalty=%g  mixture=%g  cv_rmse=%.3f  cv_rsq=%.3f",
...
extract_varimp <- function(fit_obj, year, target,
  ...
  coefs <- broom::tidy(parsnip_fit, penalty = best_penalty) %>%
    ...
    mutate(
      ...
      # question id (Q##) extracted from one-hot column name like "Q12_X3"
...
econ_varimp_2013 <- extract_varimp(econ_fit, 2013, "econ_lr_score",
                                   econ_best$penalty, econ_best$mixture, econ_cv_metrics)
anti_varimp_2013 <- extract_varimp(anti_fit, 2013, "anti_elite_score",
                                   anti_best$penalty, anti_best$mixture, anti_cv_metrics)

dir.create("../data", showWarnings = FALSE)
write.csv(econ_varimp_2013, "../data/varimp_2013_econ.csv", row.names = FALSE)
write.csv(anti_varimp_2013, "../data/varimp_2013_anti.csv", row.names = FALSE)

# Per-question summary: total absolute coefficient mass per Qxx
econ_q_summary_2013 <- econ_varimp_2013 %>%
...
anti_q_summary_2013 <- anti_varimp_2013 %>%
  filter(!is.na(question)) %>%
  group_by(question) %>%
  summarise(total_abs_coef = sum(abs_coef, na.rm = TRUE),
            n_levels_kept  = sum(coefficient != 0, na.rm = TRUE),
...
write.csv(econ_q_summary_2013, "../data/varimp_2013_econ_by_question.csv", row.names = FALSE)
write.csv(anti_q_summary_2013, "../data/varimp_2013_anti_by_question.csv", row.names = FALSE)

message("[2013] saved variable-importance tables to ../data/varimp_2013_*.csv")
...
if (.use_partei) {
  ...
  sanity_2013 <- bind_rows(econ_sanity, anti_sanity)
  write.csv(sanity_2013, "../data/imputation_sanity_2013.csv", row.names = FALSE)
  flagged <- sanity_2013 %>% filter(flag_drift | flag_signflip)
  if (nrow(flagged) > 0) {
    warning(sprintf("[2013 imputation] %d party x target cell(s) flagged; see ../data/imputation_sanity_2013.csv",
                    nrow(flagged)))
    print(flagged)
  } else {
    message("[2013 imputation] sanity check: no per-party drift or sign-flip flagged.")
  }
} else {
  message("[2013 imputation] legacy mode: skipping per-party sanity check (partei_label not used as predictor).")
}
