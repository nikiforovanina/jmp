...
wen_answers <- read.csv('wen_waehlen_btw17_answers_wide.csv')

# wen_merged is produced by 0b_data_complement_2017_cand_survey_wen.R
load("wen_merged_2017.Rdata")
...
# identity, not just survey answers. Without it, the elastic net pulled
# small/extreme parties (e.g. AfD, LINKE) toward the population mean.
...
write.csv(econ_hp, "../data/hyperparams_2017_econ.csv", row.names = FALSE)
write.csv(anti_hp, "../data/hyperparams_2017_anti.csv", row.names = FALSE)
message(sprintf("[2017 econ] best: penalty=%g  mixture=%g  cv_rmse=%.3f  cv_rsq=%.3f",
                econ_hp$penalty, econ_hp$mixture, econ_hp$cv_rmse, econ_hp$cv_rsq))
message(sprintf("[2017 anti] best: penalty=%g  mixture=%g  cv_rmse=%.3f  cv_rsq=%.3f",
...
econ_varimp_2017 <- extract_varimp(econ_fit, 2017, "econ_lr_score",
                                   econ_best$penalty, econ_best$mixture, econ_cv_metrics)
anti_varimp_2017 <- extract_varimp(anti_fit, 2017, "anti_elite_score",
                                   anti_best$penalty, anti_best$mixture, anti_cv_metrics)

dir.create("../data", showWarnings = FALSE)
write.csv(econ_varimp_2017, "../data/varimp_2017_econ.csv", row.names = FALSE)
write.csv(anti_varimp_2017, "../data/varimp_2017_anti.csv", row.names = FALSE)

econ_q_summary_2017 <- econ_varimp_2017 %>%
...
anti_q_summary_2017 <- anti_varimp_2017 %>%
  filter(!is.na(question)) %>%
  group_by(question) %>%
  summarise(total_abs_coef = sum(abs_coef, na.rm = TRUE),
            n_levels_kept  = sum(coefficient != 0, na.rm = TRUE),
...
write.csv(econ_q_summary_2017, "../data/varimp_2017_econ_by_question.csv", row.names = FALSE)
write.csv(anti_q_summary_2017, "../data/varimp_2017_anti_by_question.csv", row.names = FALSE)

message("[2017] saved variable-importance tables to ../data/varimp_2017_*.csv")
...
if (.use_partei) {
  ...
  sanity_2017 <- bind_rows(econ_sanity, anti_sanity)
  write.csv(sanity_2017, "../data/imputation_sanity_2017.csv", row.names = FALSE)
  flagged <- sanity_2017 %>% filter(flag_drift | flag_signflip)
  if (nrow(flagged) > 0) {
    warning(sprintf("[2017 imputation] %d party x target cell(s) flagged; see ../data/imputation_sanity_2017.csv",
                    nrow(flagged)))
    print(flagged)
  } else {
    message("[2017 imputation] sanity check: no per-party drift or sign-flip flagged.")
  }
} else {
  message("[2017 imputation] legacy mode: skipping per-party sanity check (partei_label not used as predictor).")
}
...
save(wen_merged_with_imputations, file = "wen_merged_with_imputations_2017.Rdata")
