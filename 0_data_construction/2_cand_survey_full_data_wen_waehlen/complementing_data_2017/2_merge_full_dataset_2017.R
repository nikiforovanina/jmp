...
load(sprintf("../../1_cand_scores_GESIS/data/cand17_scored_%s.Rdata", .cand_suffix))
load('wen_merged_with_imputations_2017.Rdata')
...
cand17_scored_pid <- cand17_scored %>%
  mutate(
...
cand17_scored_full <- rbind(cand17_scored_pid, wen_to_merge)
...
cand17_scored_full <- cand17_scored_full |>
  dplyr::left_join(party_labels, by = "party_id") |>
  dplyr::select(-party_id)  # keep only the new 'partei' column

save(cand17_scored_full, file = "../data/cand17_scored_full.Rdata")
...
shrink_to_party <- function(df_scored,
  ...
  # When supplied, the shrink anchor comes from this table instead of being
  # computed from `df_scored`. Used to anchor on direct GESIS medians only.
...
# ---------- apply shrink & save ----------
# (3) Compute shrink anchor from direct GESIS scores only — keeps the
# anchor independent of imputed values (prevents the feedback loop).
direct_medians <- cand17_scored_pid %>%
  group_by(party_id) %>%
  summarise(
    econ_med  = median(econ_lr_score,    na.rm = TRUE),
    elite_med = median(anti_elite_score, na.rm = TRUE),
...
cand17_scored_full_shrunk <- shrink_to_party(
  df_scored   = cand17_scored_full,
  lut         = party_lut_codes,
  max_dev     = 0.5,
  lambda_with = 0.75,
  clamp_min   = -1.5,
...
save(cand17_scored_full_shrunk,   file = "../data/cand17_scored_full_shrunk.Rdata")
...
cand17_scored_full_shrunk <- cand17_scored_full_shrunk %>%
...
cand17_scored_plot <- ggplot(
  cand17_scored_full_shrunk,
  aes(econ_lr_score, anti_elite_score, color = factor(party_id))
) +
  geom_point(size = 2, alpha = 0.8, na.rm = TRUE) +
  coord_cartesian(xlim = c(-1.75, 1.75), ylim = c(-1.75, 1.75)) +
  labs(
    x = "Left–Right",
    y = "Anti-Elite (↑)",
