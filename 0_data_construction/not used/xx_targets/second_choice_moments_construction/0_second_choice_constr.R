...
gesis_13_AfD <- gesis_13_AfD %>% group_by(bundesland) %>% mutate(N_Land = n())
gesis_13_AfD <- gesis_13_AfD[c('bundesland', 'N_Land', 'second_choice')]
gesis_13_AfD <- gesis_13_AfD %>% filter(!is.na(second_choice))
gesis_13_AfD <- gesis_13_AfD %>% group_by(bundesland, second_choice) %>%
  summarize(share_party = round(n()/N_Land, 2), N_Land = N_Land)
gesis_13_AfD <- unique(gesis_13_AfD)
...
gesis_17_AfD <- gesis_17_AfD %>% group_by(bundesland) %>% mutate(N_Land = n())
gesis_17_AfD <- gesis_17_AfD[c('bundesland', 'N_Land', 'second_choice')]
gesis_17_AfD <- gesis_17_AfD %>% filter(!is.na(second_choice))
gesis_17_AfD <- gesis_17_AfD %>% group_by(bundesland, second_choice) %>% 
  summarize(share_party = round(n()/N_Land, 2), N_Land = N_Land)
gesis_17_AfD <- unique(gesis_17_AfD)
...
gesis_21_AfD <- gesis_21_AfD %>% group_by(bundesland) %>% mutate(N_Land = n())
gesis_21_AfD <- gesis_21_AfD[c('bundesland', 'N_Land', 'second_choice')]
gesis_21_AfD <- gesis_21_AfD %>% filter(!is.na(second_choice))
gesis_21_AfD <- gesis_21_AfD %>% group_by(bundesland, second_choice) %>% 
  summarize(share_party = round(n()/N_Land, 2), N_Land = N_Land)
gesis_21_AfD <- unique(gesis_21_AfD)

gesis_13_AfD$bundesland <- ifelse(gesis_13_AfD$bundesland > 11, gesis_13_AfD$bundesland - 1, gesis_13_AfD$bundesland)
gesis_17_AfD$bundesland <- ifelse(gesis_17_AfD$bundesland > 11, gesis_17_AfD$bundesland - 1, gesis_17_AfD$bundesland)
gesis_21_AfD$bundesland <- ifelse(gesis_21_AfD$bundesland > 11, gesis_21_AfD$bundesland - 1, gesis_21_AfD$bundesland)


write.csv(gesis_13_AfD, file = 'second_choice_2013.csv', row.names = FALSE)
write.csv(gesis_17_AfD, file = 'second_choice_2017.csv', row.names = FALSE)
write.csv(gesis_21_AfD, file = 'second_choice_2021.csv', row.names = FALSE)
