...
dat_short <- dat[c('bula', 'wknr', 'q35b', 'q35c', 'q35d', 'q35e', 'q35f', 'q35g', 'q35h', 'q75a')]
...
dat_short <- dat %>% 
  select(lfdn, bula, wknr, q35b, q35c, q35d, q35e, q35f, q35g, q35h, q75a)
...
long_q35 <- dat_short %>%
  ...
  select(lfdn, bula, wknr, party, party_pos, chosen)
...
as.numeric(long_q35$wknr[1:100])

as.numeric(long_q35$wknr[1125])
...
m1 <- felm(party_pos ~ econ_lr_score| bula + partei|0|0, data = merged )
m2 <- felm(party_pos ~ econ_lr_score| bula + partei|0|0, data = merged %>%  filter(chosen == 1) )
m3 <- felm(party_pos ~ econ_lr_score| bula + partei|0|0 , data = merged %>%  filter(chosen == 1) %>%  filter(party6 %in% c(6)) )

summary(m1)

stargazer(m1, m2, m3)

ggplot(merged, aes(econ_lr_score, party_pos)) +
  geom_point(
    alpha = 0.2, size = 1, aes(col = party),
    position = position_jitter(width = 0, height = 0.15)
  ) +
  stat_summary_bin(
    fun = mean, bins = 15, geom = "line", linewidth = 1
  ) +
  scale_y_continuous(breaks = 1:11, limits = c(0.5, 11.5)) +
  theme_bw() +
  labs(x = "Economic left–right score", y = "Perceived party position")

ggplot(merged %>% filter(chosen == 1), aes(econ_lr_score, party_pos)) +
  geom_point(
    alpha = 0.3, size = 1, aes(col = party, fill = party),
    position = position_jitter(width = 0, height = 0.15)
  ) +
  stat_summary_bin(
    fun = mean, bins = 15, geom = "line", linewidth = 1
...
m1 <- felm(party_pos ~ econ_lr_score | bula + partei, data = merged)

m2 <- felm(party_pos ~ econ_lr_score | bula + partei,
           data = merged %>% filter(chosen == 1))

m3 <- felm(party_pos ~ econ_lr_score | bula + partei,
           data = merged %>% filter(chosen == 1, party6 %in% c(6)))
...
tab_body <- stargazer(
  ...
  type = "latex",
  title = "",
  out = "perceived_estimated_pos.tex",
  dep.var.caption = "",
  dep.var.labels = "",
  column.labels = c("All Parties", "Chosen Parties", "Chosen AfD"),
  model.numbers = TRUE,
  omit.stat = c("f", "ser"),
  covariate.labels = c("Candidate, L--R"),
  keep = c("econ_lr_score"),
  add.lines = list(
...
writeLines(tab_full, "table_candidate_party_lr.tex")
