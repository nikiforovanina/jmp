...
# SPEC_TAG (e.g. "10k_relaxboth_LINKE") drives the graph filenames so different
# specifications don't overwrite each other.
source("0_configuration.R")
stopifnot(exists("SPEC_TAG"), nzchar(SPEC_TAG))

# -----------------------------
# COLORS: AboveNeg45 = green, BelowNeg45 = pink
# -----------------------------
cols <- c("AboveNeg45" = "#3CB371", "BelowNeg45" = "#FF69B4")

# -----------------------------
# Read bounds
# -----------------------------
df2013 <- read.csv("bounds/linke_bounds_year2013_seed01.csv")
df2017 <- read.csv("bounds/linke_bounds_year2017_seed01.csv")
df2021 <- read.csv("bounds/linke_bounds_year2021_seed01.csv")
...
bounds_all$region <- ifelse(bounds_all$region == 'AboveNeg45', 'Anti-Elite', 'Left')
cols <- c("Anti-Elite" = "#3CB371", "Left" = "#FF69B4")
...
#region_year_bounds$region <- ifelse(region_year_bounds$region_old == "AntiEst", "BelowNeg45", "AboveNeg45")
#region_year_bounds$region <- factor(region_year_bounds$region, levels = c("AboveNeg45", "BelowNeg45"))
...
#region_year_bounds_ew$region <- ifelse(region_year_bounds_ew$region_old == "AntiEst", "BelowNeg45", "AboveNeg45")
#region_year_bounds_ew$region <- factor(region_year_bounds_ew$region, levels = c("AboveNeg45", "BelowNeg45"))
