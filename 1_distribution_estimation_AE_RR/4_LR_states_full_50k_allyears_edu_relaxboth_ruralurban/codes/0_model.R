...
# 48 groups = 2 gender x 6 age x 2 edu x 2 area (rural/urban)
# Per-muni share columns (e.g. "female_18_24_noedu_rural_share") are built in
# the runner by splitting the original 24 *_noedu_share / *_edu_share columns
# according to the muni's eligible_voters: rural = < RURAL_MAX_ELIGIBLE.
.group_spec_24 <- tibble::tribble(
  ~base_key,        ~base_pretty,    ~base_col_pre,
  "female_1825",    "Female 18–25",  "female_18_24",
  "female_2535",    "Female 25–35",  "female_25_34",
  "female_3545",    "Female 35–45",  "female_35_44",
  "female_4560",    "Female 45–60",  "female_45_59",
  "female_6070",    "Female 60–70",  "female_60_69",
  "female_70plus",  "Female 70+",    "female_70plus",
  "male_1825",      "Male 18–25",    "male_18_24",
  "male_2535",      "Male 25–35",    "male_25_34",
  "male_3545",      "Male 35–45",    "male_35_44",
  "male_4560",      "Male 45–60",    "male_45_59",
  "male_6070",      "Male 60–70",    "male_60_69",
  "male_70plus",    "Male 70+",      "male_70plus"
)
.expand_groups <- function(tb24) {
  rows <- list()
  for (i in seq_len(nrow(tb24))) {
    bk <- tb24$base_key[i]; bp <- tb24$base_pretty[i]; bc <- tb24$base_col_pre[i]
    for (edu in c(FALSE, TRUE)) {
      edu_tag_key  <- if (edu) "_edu" else ""
      edu_tag_col  <- if (edu) "edu" else "noedu"
      edu_tag_pp   <- if (edu) "edu, " else ""
      for (area in c("rural", "urban")) {
        rows[[length(rows) + 1L]] <- tibble::tibble(
          key       = paste0(bk, edu_tag_key, "_", area),
          pretty    = sprintf("%s (%s%s)", bp, edu_tag_pp, area),
          col       = sprintf("%s_%s_%s_share", bc, edu_tag_col, area),
          tgt_name  = paste0(bk, edu_tag_key, "_", area),
          base_key  = bk,
          area      = area,
          edu       = edu
        )
...
group_spec <- .expand_groups(.group_spec_24)

# Helper: strip area + edu suffix from a group key to recover the base
# age-gender key (e.g. "female_1825_edu_rural" -> "female_1825").
strip_to_base_key <- function(gk) {
  gsub("(_edu)?_(rural|urban)$", "", gk)
}

# Helper: original 24 muni-level share columns (the ones that exist in
# the input RData files). The runner expands these to 48 rural/urban
# variants based on RURAL_MAX_ELIGIBLE.
ORIG_DEMO_COLS <- as.character(unlist(lapply(.group_spec_24$base_col_pre, function(bc) {
  c(paste0(bc, "_noedu_share"), paste0(bc, "_edu_share"))
})))
...
get_targets_48 <- function(STATE) {
  ...
  # only the 12 base age-gender keys must exist in the targets file;
  # edu and rural/urban variants inherit their base group's target,
  # weighted naturally through the T_g masses.
  base_keys <- unique(group_spec$base_key)
  ...
  for (i in seq_len(nrow(group_spec))) {
    gk <- group_spec$key[i]
    bk <- group_spec$base_key[i]
    out[[gk]] <- normalize_target_vec(tg[[bk]])
  }
  out
}
...
apply_turnout_adjustment <- function(df, STATE, group_cols) {
  ...
  for (cc in group_cols) {
    ...
    # strips _noedu_rural_share / _edu_urban_share / etc.
    age_key <- gsub("_(noedu|edu)_(rural|urban)_share$", "", age_key)
...
solve_afd_only <- function(B_AFD_list, rows_afd, rows_sc,
  ...
  # summing all 4 sub-groups (edu/noedu x rural/urban) against the shared
  # state-level base target.  Mass weighting through T_total is automatic.
  base_keys <- unique(strip_to_base_key(gkeys))
  sub_keys_for_base <- function(bg) {
    intersect(
      c(paste0(bg, "_rural"),     paste0(bg, "_urban"),
        paste0(bg, "_edu_rural"), paste0(bg, "_edu_urban")),
      gkeys
    )
  }
  for (bg in base_keys) {
    sub_keys <- sub_keys_for_base(bg)
    if (!length(sub_keys)) next
    X_bg <- as.numeric(tg_list[[ sub_keys[1] ]][6])  # all sub-keys share base target
  ...
  # Second-choice block: LEFT only — PAIRED per base age-gender group across
  # all 4 sub-groups (edu/noedu x rural/urban).  SC data is measured at the
  # state-pair level, so one paired constraint per base group is applied,
  # summing all sub-groups' Pi vectors against combined mass T_total.
  ...
  for (bg in base_keys) {
    ...
    T_total <- sum(group_masses[sub_keys], na.rm = TRUE)
    if (!is.finite(T_total) || T_total <= 0) next
    X_g <- as.numeric(tg_list[[ sub_keys[1] ]][6])
  ...
  # ---- Edu/non-edu similarity, applied WITHIN each area (rural pairs with
  #      rural, urban with urban).  Rural and urban Pi are intentionally not
  #      tied — that is the whole point of the rural/urban split.
  for (bg in base_keys) {
    for (area in c("rural","urban")) {
      base_key <- paste0(bg, "_", area)
      edu_key  <- paste0(bg, "_edu_", area)
      if (!(base_key %in% gkeys) || !(edu_key %in% gkeys)) next
      cons <- c(cons, list(
        Pi[[base_key]] <= (1 + EDU_DIFF_TOL) * Pi[[edu_key]],
        Pi[[edu_key]]  <= (1 + EDU_DIFF_TOL) * Pi[[base_key]]
      ))
    }
  }
...
run_one <- function(STATE, seed_base = 0, seed_idx = 1) {
  ...
  # Demography — start from the 24 ORIGINAL demographic columns that exist
  # in the input data, then split into 48 (rural / urban) variants below.
  miss_orig <- setdiff(ORIG_DEMO_COLS, names(results_std))
  if (length(miss_orig)) {
    warning(sprintf(
      "[STATE %02d] Missing %d/%d ORIGINAL demographic columns — filling with 0.\n  Missing: %s\n  Available in data: %s",
      STATE, length(miss_orig), length(ORIG_DEMO_COLS),
      paste(miss_orig, collapse=", "),
      paste(intersect(ORIG_DEMO_COLS, names(results_std)), collapse=", ")
    ))
    results_std[miss_orig] <- 0
  }
  # Split each *_share column into *_rural_share and *_urban_share based on
  # the muni's eligible_voters threshold (RURAL_MAX_ELIGIBLE).  For rural
  # munis the urban variants are 0; for urban munis, vice versa.  This
  # preserves the row-sum invariant across the new 48 columns.
  if (!"eligible_voters" %in% names(results_std)) {
    stop(sprintf("[STATE %02d] eligible_voters column required for rural/urban split", STATE))
  }
  is_rural <- as.numeric(results_std$eligible_voters) < RURAL_MAX_ELIGIBLE
  is_rural[is.na(is_rural)] <- FALSE
  for (cc in ORIG_DEMO_COLS) {
    rural_col <- sub("_share$", "_rural_share", cc)
    urban_col <- sub("_share$", "_urban_share", cc)
    v <- as.numeric(results_std[[cc]])
    results_std[[rural_col]] <- ifelse(is_rural, v, 0)
    results_std[[urban_col]] <- ifelse(is_rural, 0, v)
  }
  message(sprintf("[STATE %02d, year %d] Rural/urban split: %d rural munis, %d urban munis (threshold = %d)",
                  STATE, YEAR, sum(is_rural), sum(!is_rural), RURAL_MAX_ELIGIBLE))

  need_cols <- group_spec$col
  miss <- setdiff(need_cols, names(results_std))
  if (length(miss)) {
    stop(sprintf(
      "[STATE %02d] Internal error: %d/%d expanded rural/urban columns missing after split: %s",
      STATE, length(miss), length(need_cols), paste(miss, collapse=", ")
    ))
  }
  ...
  meta_groups <- list(
    all = group_spec$key  # all 48 groups (12 base x edu/noedu x rural/urban)
  )
  meta_pretty <- c(
