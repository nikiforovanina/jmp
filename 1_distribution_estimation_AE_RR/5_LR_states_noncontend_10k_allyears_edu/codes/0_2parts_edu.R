# ===============================================================
# AfD-only model with Linke-only second-choice constraints
# and a counterfactual 2-party system with AfD-median-based positions:
#   - Right   at (median_AfD_x - 1, median_AfD_y)
#   - AntiEst at (median_AfD_x,     median_AfD_y - 1)
#
# Geometry for baseline:
#   - 6-party full-plane Voronoi winner regions.
#   - Within AfD winner region, carve subregions where
#     the second choice is:
#         * LEFT   = LINKE (5)
#     The remainder is sc=0.
#   - Superimpose partitions across elections → cells c with (p_e, sc_e).
#   - Keep ONLY cells where AfD wins in at least one election.
#
# Optimisation (baseline):
#   - Objective: min L1 discrepancy of municipal AfD shares.
#   - Constraints:
#       (1) Group-level AfD share ≈ representative stat X^d (per group).
#       (3) Second-choice among AfD voters, in TWO buckets:
#           * LEFT     = LINKE (party 5)        → constrained
#           * OTHER    = anything else          → residual / unconstrained
#
# Counterfactual 2-party mapping (for AfD voters only, BY OVERLAP):
#   - Ignore original 6 parties for this step and imagine two new parties:
#       * Right   at R = (median_AfD_x - 1, median_AfD_y)
#       * AntiEst at A = (median_AfD_x,     median_AfD_y - 1)
#   - Build the 2-party Voronoi diagram on a bounding rectangle that
#     contains both original-party and counterfactual positions.
#   - For each AfD mesh cell c and each counterfactual party P ∈ {Right,
#     AntiEst}, define:
#         mask_P(c) = 1  if Voronoi region of P intersects cell c
#                    0  otherwise
#     A cell can have mask_P(c)=1 for multiple P: any overlap is allowed.
#   - In the bounds, a cell with mask_P(c)=1 can have its entire AfD mass
#     assigned to party P.
#
# Bounds:
#   - SINGLE META-GROUP:
#       * all = all 24 demographic groups (12 age-gender × 2 edu) summed
#   - For this meta-group and "party-region":
#       * region = "Right"
#       * region = "AntiEst"
#   - Let T_all be total AfD-voter "mass" for all groups.
#   - We search b ∈ [0, 0.5] such that:
#       * upper bound for region R:  mass_all(R) ≥ b * T_all is feasible
#       * lower bound for region R:  mass_all(R) ≤ b * T_all is feasible
#     where mass_all(R) is the sum over AfD mass in cells with mask_R(c)=1,
#     aggregated over all 24 groups.
#   - Feasibility: MAE ≤ baseline MAE (t_bench) up to ABS_EPS / REL_EPS.
#
# Outputs:
#   - fits_afd_only/afd_only_state%02d_year%d.rds : baseline AfD fit.
#   - bounds/afd_bounds_year%d.csv                : 2-party bounds by state.
#   - diagnostics/afd_only_two_party_cell_counts_state%02d_year%d.csv :
#       cell counts per region (Right/AntiEst).
# ===============================================================
...
# Tolerances
AFD_GROUP_TOL      <- 0.005   # tolerance on group-level AfD share X^d (absolute)
SC_TOL             <- 0.05   # tolerance on second-choice shares
EDU_DIFF_TOL       <- 0.20    # max allowed relative difference in Pi[c] between edu and non-edu
                               # sub-groups of the same age-gender group (element-wise per cell)
...
SEED_LIST <- c(1509)  # 5 seeds to try

# Voronoi padding for geometry
VOR_PAD            <- 5

# Tiny-cell merging: cells whose area / bbox_area < TINY_CELL_FRAC_EPS are
# absorbed into the touching neighbor with the largest area.
TINY_CELL_FRAC_EPS <- 1e-5

# AfD second-choice structure (base parties)
# Track LINKE (5) as the ONLY explicit AfD second choice.
SC_PARTIES         <- c(5L)  # LEFT only

ww_cand <- read.csv("../../data/candidates_with_erststimmen.csv")

ww_cand <- ww_cand %>%
  group_by(year, wk_num) %>%
  mutate(
    top_pct   = max(erststimme_pct, na.rm = TRUE),
    gap       = top_pct - erststimme_pct,
    contender = as.integer(erststimme_winner == 1 | gap < 5)
  ) %>%
  ungroup()

# Wahlkreise where AfD was a contender (winner OR within 5pp of the top vote share)
# in this YEAR. These are dropped from the estimation sample.
CONTENDER_WK <- ww_cand %>%
  filter(party_label == 'AfD', year == YEAR, contender == 1) %>%
  pull(wk_num) %>%
  unique()
message(sprintf(">>> AfD-contender Wahlkreise dropped (year %d): %d",
                YEAR, length(CONTENDER_WK)))
...
run_one <- function(STATE, seed_base = 0, seed_idx = 1) {
  ...
  results_std <- results_std %>% filter(!election_id %in% CONTENDER_WK)
  pos_std     <- pos_std     %>% filter(!election_id %in% CONTENDER_WK)
